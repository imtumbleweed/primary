...
class FacebookAuthService {
  ...
  endpointUrl() {
    const url =
      ...
      `redirect_uri=${this.redirect_uri}&state=${this.state}`;
    return url;
  }
   ...
   */
  tokenUrl(code) {
		const url = `https://graph.facebook.com/v5.0/oauth/access_token?`+
  ...
  async verifyToken(token) {
    try {
			...
			`input_token=${token}&access_token=${this.client_id}|${this.client_secret}`;
      const response = await fetch(url);
