...
export class Slider {
  ...
  static initialize() {
      console.log("Initializing slider UI (convert all '.slider' to a slider UI control)");
        document.querySelectorAll(".slider").forEach(function(obj) {
            let id = obj.getAttribute("id");
            let width = 200;
            let height = 16;
            new Slider(window.mouse, "slider_id_" + id, id, "name" + Math.floor(Math.random() * 100), {x:0,y:0}, HORIZONTAL, width, height, 0, 200, 0, false, false);
        });
  }

  constructor(mouse, // globally available mouse object
